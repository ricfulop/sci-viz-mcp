# GPT Image Prompt — Sci-Viz MCP Repo Graphic

Create a wide hero graphic for a GitHub README for a software repo called
"Sci-Viz MCP". The repo lets AI agents in an IDE control headless engineering
design and scientific visualization tools.

Composition:
- Dark, premium engineering-lab background with a subtle IDE/code grid.
- Center: glowing "Sci-Viz MCP" wordmark, clean modern sans-serif, crisp and
  readable.
- Surrounding the wordmark: five connected tool panels/nodes forming an
  engineering design constellation:
  1. COMSOL-style multiphysics field map: thermal gradient + electromagnetic
     field lines + structural mesh.
  2. 2D ray-optics telescope design: yellow/green ray bundles through lenses
     and mirrors toward a focal plane.
  3. Crystal structure visualization: ball-and-stick unit cell with lattice
     vectors.
  4. Atomistic rendering: dense particles with coordination colors / OVITO
     scientific render feel.
  5. Blender 3D rendering: camera, lights, and polished 3D object silhouette.
- Include small badges for "APS" and "Nature" figure styles as clean journal
  style tags.
- Add a tiny footer text: "Designed with Sci-Viz (c) 2026 Ric Fulop, MIT Center
  for Bits and Atoms".

Style:
- Technical but beautiful, not cartoonish.
- High-contrast, crisp linework, luminous accents in cyan, amber, violet, and
  green on a near-black background.
- No fake UI clutter, no unreadable paragraphs, no stock-photo people.
- GitHub README hero, landscape aspect ratio, 1536×1024 or similar.
